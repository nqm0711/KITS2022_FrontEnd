...
function StudentTable(student) 
{
  ...
  const list=studentData.map((item)=><tr key={item.id} className={item.mark>9?"table-success":(item.mark>=8?"table-primary":(item.mark>=6?"table-warning":"table-danger"))}><td>{item.name}</td><td>{item.age}</td><td>{item.email}</td><td>{item.mark}</td><td>{<User data={item}/>}</td></tr>);
