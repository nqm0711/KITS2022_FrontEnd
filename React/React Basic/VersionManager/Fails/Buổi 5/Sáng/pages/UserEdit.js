...
/*    useEffect(()=>{
    fetch("https://62b0495de460b79df0422035.mockapi.io/countries")
...
/*  useEffect(() => {
  ...
  if (countryList != null){
    const countryOptions = countryList.map((item)=>(<option value={item.country}>{item.country}</option>));
    console.log("option",countryOptions)
  }*/
  ...
  const saveUser = () => {
    ...
    fetch(
      'https://62b0495de460b79df0422035.mockapi.io/student' + student.id,
      requestOptions
    )
      .then((response) => response.json())
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          <select onChange={(e)=> setHome(student.home.address,student.home.city,e.target.value)}>
                            {countryList.map((item)=>(<option key={item.id} value={item.country}>{item.country}</option>))}
                          </select>
